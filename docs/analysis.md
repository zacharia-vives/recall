...
| R2.10 | The camera screen fits on one screen. The room it may use is measured to the top edge of the pill, not by subtracting the pill's height, because a floating pill's footprint is its height plus its gap plus whatever the phone reserves | Must |
| R4.9 | The selection slides between the three ways in rather than jumping. Its own element, measured from the current tab's own box so it holds in any language, decorative only: which tab is current is carried by aria-current and by a plain background until the lozenge has been measured, and somebody who has asked for less movement gets it without the travel | Should |
| R4.8 | Every control follows the skin, including the ones the browser draws itself. The zoom slider was the default blue in all twenty four until a photograph showed it | Must |
