...
| **Not a recipient: whoever hosts the type** | The twenty four skins added on 9 September use nine typefaces, and the files are served from the app itself | **Deliberately not a transfer.** The easy way to add a typeface is a link to a font host, which hands that host the IP address of every reader on every visit, and dynamic Google Fonts embedding is the subject of a German ruling on exactly that. So the woff2 files sit in the repository under their own open licences. No new third party was introduced by any of this, and a phone on the default skin downloads no type at all |
...
| A nicer look is bought by calling a font host on every visit | The keeper | Medium if done the easy way | The nine faces are served from the app itself, under their open licences, so nothing third party is contacted for type. Checked by measuring what actually leaves the origin: with all nine faces exercised, the count of foreign requests did not move. The only one there is the library finding below, which predates this | Low |
| A nicer voice is bought by sending her letters to a server | The keeper | Medium | The neural voice added on 9 September runs on the device, in WebAssembly, and the model is fetched once and then kept locally. So the app sounds better without giving up the rule below. Fetching it is the only network call the feature makes, and it is disclosed in the notice in all three languages | Low |
| Sixty megabytes is taken off somebody's data plan without asking | The keeper, and whoever pays the bill | Medium once the voice fetches itself | The voice is fetched on its own only where the browser says the connection can carry it and data saving is off. On anything slower it waits, says it is waiting for wifi, and offers a button that states the size. The phone's own voice reads in the meantime | Low |
