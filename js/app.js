...
import * as speech from "./speech.js?v=49";
import * as camera from "./camera.js?v=49";
import * as ocr from "./ocr.js?v=49";
import { isConfigured, NOTICE_VERSION } from "./config.js?v=49";
import * as install from "./install.js?v=49";
import * as lock from "./lock.js?v=49";
import * as i18n from "./i18n.js?v=49";
import * as docs from "./docs.js?v=49";
import * as skins from "./skins.js?v=49";
...
async function showWhoHasAccess() {
  ...
  try {
    const cloud = await import("./cloud.js?v=49");
...
async function syncHousehold(options) {
  ...
  let cloud;
  try {
    cloud = await import("./cloud.js?v=49");
  } catch (err) {
...
function fitCameraScreen() {
  ...
  // Measure to the top of the pill rather than subtracting its height. The
  // pill floats now, so its real footprint is its height plus the gap below
  // it plus whatever the phone reserves, and a photograph from an iPhone
  // showed the difference: the two photo buttons sat underneath it, cut off.
  // Its own top edge already knows all three numbers. R2.10.
  const pill = tabs.getBoundingClientRect();
